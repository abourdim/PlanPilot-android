#!/usr/bin/env bash
# ============================================================================
#  PlanPilot Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/app-tier-s-PlanPilot}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.planpilot}"
AVD_NAME="${AVD_NAME:-PlanPilot_test}"
...
cmd_test() {
  ...
  local out="/tmp/PlanPilot-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   PlanPilot Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
